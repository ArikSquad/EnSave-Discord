...
class SongQueue(asyncio.Queue):
    def __getitem__(self, item):
        if isinstance(item, slice):
            return list(itertools.islice(Music._queue, item.start, item.stop, item.step))
        else:
            return Music._queue[item]

    def __iter__(self):
        return Music._queue.__iter__()
    ...
    def shuffle(self):
        random.shuffle(Music._queue)

    def remove(self, index: int):
        del Music._queue[index]
